            ...
            if (send) { //send to server
                ...
                for(var name in this.options.params) {
                    if (this.options[name]!==undefined) {
                        ...
                        //alert('value vorher ' + this.options.params[name]);
                        this.options.params[name] = this.options[name];
                        //alert('value nachher ' + this.options.params[name]);
                    }
                }
                // finish added code.
            ...
            case 'submit':  //collects value, validate and submit to server for creating new record
                ...
                if($.isEmptyObject(errors)) {
                    ...
                    if($elems.length === 1) {
                        ...
                        // added by BHoft
                        // set the options.params.attribute with the appropriate options.attribute if existing
                        for(var name in editable.options.params) {
                            if (editable.options[name]!==undefined) {
                                //alert('editable name ' + name);
                                //alert('value vorher ' + editable.options.params[name]);
                                editable.options.params[name] = editable.options[name];
                                //alert('value nachher ' + editable.options.params[name]);
                            }
                        }
                        // finish added code.
